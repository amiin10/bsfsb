#!/usr/bin/env python3
"""
Recover the KdV PDE parameters (lambda_1, lambda_2) with ePINN-AF from
noisy measurements of u, and save results.

Usage:
    python train.py --data /path/to/KdV.mat
    python train.py --data /path/to/KdV.mat --noise 2 --n-iter 10000
"""
import argparse
import pickle
import time
...
def main():
    ...
    ap.add_argument('--data', required=True, help='Path to KdV.mat')
    ...
    ap.add_argument('--out', default='kdv_inverse_epinn_af_results.pkl')
    args = ap.parse_args()
    ...
    X_train, u_train = sample_training_set(
    ...
    print(f'Recovered lambda_2 = {l2_hat:.6f}  (true {LAMBDA_2_TRUE}, error {err_l2:.3f}%)')
