"""
ePINN-AF training wrapper for the INVERSE Korteweg-de Vries (KdV) problem:
given noisy supervised data on `u`, simultaneously learn the network AND
the two PDE coefficients lambda_1 (advection) and lambda_2 (dispersion).

PDE (with TRAINABLE lambda_1, lambda_2):
    u_t + lambda_1 * u * u_x + lambda_2 * u_xxx = 0

True values used to generate data: lambda_1 = 1.0, lambda_2 = 0.0025

Loss (Eq 3.22): L = MSE_u + MSE_f. No adaptive weighting — the
fuzzy-attention mechanism inside the network itself handles adaptive
focus (Eq 3.10).
"""
import numpy as np
import torch
...
class PhysicsInformedNN:
    """ePINN-AF training wrapper for the inverse KdV problem.

    Args:
        X: training coordinates [N, 2] with columns [x, t]
        u: training (noisy) solution values [N, 1]
    ...
    def __init__(self, X, u, backbone_layers, n_rules, attn_hidden, lb, ub):
        ...
        # ---- INVERSE-PROBLEM trainable PDE coefficients ----
        self.lambda_1 = torch.nn.Parameter(torch.tensor([0.0], device=self.device))
        self.lambda_2 = torch.nn.Parameter(torch.tensor([0.0], device=self.device))
        ...
        self.lambda_history = {'lambda_1': [], 'lambda_2': []}
    ...
    def net_f(self, x, t):
        ...
        u_xxx = torch.autograd.grad(u_xx, x, grad_outputs=torch.ones_like(u_xx),
                                    retain_graph=True, create_graph=True)[0]
        return u_t + self.lambda_1 * u * u_x + self.lambda_2 * u_xxx
    ...
    def loss_fn(self):
        ...
        self.lambda_history['lambda_1'].append(self.lambda_1.item())
        self.lambda_history['lambda_2'].append(self.lambda_2.item())
        return loss
        ...
        for epoch in range(n_iter):
            ...
            if epoch % 1000 == 0:
                print(f'Adam Epoch: {epoch}, Loss: {loss.item():.4e}, '
                      ...
                      f'lam1: {self.lambda_1.item():.5f}, lam2: {self.lambda_2.item():.6f}')
    ...
    def predict(self, X):
        """Predict solution and PDE residual for given coordinates."""
