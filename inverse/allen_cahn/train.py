#!/usr/bin/env python3
"""
Recover the Allen-Cahn PDE parameters (lambda_1, lambda_2) with ePINN-AF
from noisy measurements of u, and save results.

Usage:
    python train.py --data /path/to/AC.mat
    python train.py --data /path/to/AC.mat --noise 3 --n-iter 10000
"""
import argparse
import pickle
import time
...
def main():
    ...
    ap.add_argument('--out', default='allen_cahn_inverse_epinn_af_results.pkl')
    args = ap.parse_args()
    ...
    print(f"True parameters: lambda_1={LAMBDA_1_TRUE}, lambda_2={LAMBDA_2_TRUE}")
    print(f"Noise level: {args.noise}%")

    X_train, u_train_noisy = sample_training_set(
        X_star, u_star, args.n_train, args.noise, seed=args.seed)


    pinn = PhysicsInformedNN(
        X_train, u_train_noisy,
    ...
    max_error = float(np.max(np.abs(u_star - u_pred)))
    ...
    print(f'Recovered lambda_1 = {l1_hat:.6e}  (true {LAMBDA_1_TRUE}, error {err_l1:.3f}%)')
    print(f'Recovered lambda_2 = {l2_hat:.4f}  (true {LAMBDA_2_TRUE}, error {err_l2:.3f}%)')

    results = {
        'noise_pct': args.noise,
        'training_time': training_time,
        'error_u': error_u, 'mse_f': mse_f, 'max_error': max_error,
        ...
        'X_train': X_train, 'u_train': u_train_noisy,
        'lb': lb, 'ub': ub,
