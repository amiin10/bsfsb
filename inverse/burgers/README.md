# ePINN-AF — Burgers equation (inverse / parameter identification)
...
True values: `lambda_1 = 1.0`, `lambda_2 = 0.01/pi`. Both are trained
...
python train.py --data /path/to/burgers_shock.mat --noise 3
```

`--noise` is the measurement-noise level, in percent of `std(u)` (default
0). Other flags: `--n-iter` (default 10000), `--n-train` (default 2000),
`--seed`.
