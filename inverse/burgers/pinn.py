"""
ePINN-AF training wrapper for the INVERSE Burgers problem: given noisy
supervised data on `u`, simultaneously learn the network AND the two PDE
coefficients lambda_1 (advection) and lambda_2 (viscosity).

PDE (with TRAINABLE lambda_1, lambda_2):
    u_t + lambda_1 * u * u_x - lambda_2 * u_xx = 0

True values used to generate data: lambda_1 = 1.0, lambda_2 = 0.01/pi
"""
import numpy as np
import torch
...
class PhysicsInformedNN:
    """ePINN-AF wrapper for the inverse Burgers problem.
    Loss (Eq 3.22): L = MSE_u + MSE_f
    """
    def __init__(self, X, u, X_f, input_dim, backbone_layers, n_rules, attn_hidden, lb, ub):
        ...
        # Initialised to zero (Raissi et al. 2019 convention). Sign
        # convention matches the residual u_t + lambda_1*u*u_x - lambda_2*u_xx = 0.
        ...
        # Both lambdas are jointly optimised with the network weights.
        ...
        self.optimizer_LBFGS = torch.optim.LBFGS(
            all_params,
            lr=1.0, max_iter=50000, max_eval=50000, history_size=100,
            tolerance_grad=1e-7,
    ...
    def net_f(self, x, t):
        """Burgers residual (inverse): u_t + lambda_1*u*u_x - lambda_2*u_xx = 0"""
        u = self.net_u(x, t)
