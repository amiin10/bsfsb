"""
ePINN-AF network architecture — Poisson boundary value problem (2D or 3D).

Uses the per-rule output-head formulation (same family as KdV/NS):
    u_hat(z) = sum_j alpha_j(z) mu_j(z) h_j(z) + b,   h_j(z) = W_j . h(z)
with inputs normalised to [-1, 1]^d via the domain bounds (lb, ub).
"""
import torch
...
class AFPINN(torch.nn.Module):
    """ePINN-AF (Eq 3.10):  u_hat(z) = sum_j alpha_j(z) mu_j(z) h_j(z) + b
       with per-rule heads h_j = W_j . h(z) over a shared backbone.
       Domain bounds (lb, ub) map inputs into [-1, 1]^d.
    """
    def __init__(self, backbone_layers, n_rules, attn_hidden,
                 output_dim=1, input_dim=2,
                 sigma_lo=0.1, sigma_hi=1, lb=None, ub=None):
        super().__init__()
        self.n_rules = n_rules
        self.input_dim = input_dim
        ...
        sizes = [input_dim] + backbone_layers
        layers = []
    ...
    def forward(self, z, return_components=False):
        ...
        mu, alpha, gamma = self.fuzzy_attention(z_n)
