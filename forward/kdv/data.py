"""
Data loading for the forward KdV benchmark.

Expects a `KdV.mat` file with:
    x  : [N, 1]   spatial grid
    tt : [T, 1]   time grid
...
def load_data(data_path, n_train=8000, seed=None):
    """Load KdV.mat and build the training set.

    Returns a dict with x, t, Exact, X_star, u_star, lb, ub, X_train, u_train.
    Note: this PDE setup uses the SAME points for the data-fidelity term and
    the PDE-residual term (no separate, larger collocation set).
    """
