#!/usr/bin/env python3
"""
Train ePINN-AF on the forward Burgers equation and save results.

Usage:
    python train.py --data /path/to/burgers_shock.mat
    python train.py --data /path/to/burgers_shock.mat --n-iter 10000
"""
import argparse
import pickle
import time
...
def main():
    ...
    ap.add_argument('--out', default='burgers_epinn_af_results.pkl')
    args = ap.parse_args()
    ...
    X_star, u_star, X_f = d['X_star'], d['u_star'], d['X_f']
    lb, ub = d['lb'], d['ub']
    ...
    print(f"Total points: {X_star.shape[0]}, training points: {args.n_train}, "
          f"collocation points: {X_f.shape[0]}")
